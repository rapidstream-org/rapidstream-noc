...
def vp1802_nodes(
    ...
    for x in range(num_col):
        ...
        for y in range(num_inter_rows):
            col.append(Node(name=f"nps_x{x}y{y}"))
...
def vp1802_nocgraph(num_slr: int, num_col: int, rows_per_slr: list[int]) -> NocGraph:
    ...
    >>> G = vp1802_nocgraph(num_slr, num_col, rows_per_slr)
    >>> nx_g = nx.DiGraph()
